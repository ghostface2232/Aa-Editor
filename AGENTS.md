...
- The status bar's line count, its caret row, and Go to Line all use the *logical line* defined in `src/utils/documentLines.ts`: one line per textblock plus one per newline character or hard break inside it. A jump target goes through `selectionForLinePos` — a line that *is* a leaf block resolves to a block position, where `TextSelection.create` does not throw but yields a selection whose parent has no inline content, so the next keystroke inserts a stray paragraph. That fallback assumes every block leaf in the schema is selectable; a non-selectable block atom would silently jump to the wrong line. Counting top-level blocks (`doc.childCount`) instead made a ten-item list and a twenty-line code block read as one line. Keep the three readouts on the same definition — the number the status bar shows must be the number Go to Line accepts. `buildLineIndex` is O(document), so cache it against `doc` identity (selection-only transactions reuse the node), never rebuild it per caret move, and coalesce doc-change rebuilds to one per frame — both the status bar and Go to Line subscribe to every transaction. The same walk produces the character and word counts: taking them from `doc.textContent` instead both allocates a second copy of the document and fuses each block's last word to the next block's first.
...
- Those shortcuts act on the note the user pointed at, in this order: the right-clicked note while a note context menu is open, else the row that took the last mousedown/focusin, else the active note. The pointed row is remembered as a note **id** and resolved to an index at keypress time, because sorting can move a row in between (and a row that has since disappeared must resolve to nothing rather than to whatever now sits at that index). Targeting the active note instead would act on the open note while the user is looking at a different, focused row.
