...
- `useWindowSync` broadcasts note, group, pin, color, and trash changes between Tauri windows. Remote updates leave dirty local documents untouched. Remote deletion always wins: the id is quarantined from every autosave entry point before the live editor is removed, all already in-flight snapshot bodies remain cleanup candidates, and close/migration drains wait for preservation to finish. A dirty local body is folded into the matching trash body when possible, while only a divergent or unwritable trash version falls back to `.conflicts/`; preservation failure is logged but never resurrects the deleted note. When deletion provisions a new last note, the source broadcasts it only after its body exists, and a peer activates it whether the create event arrives before or after the delete event.
