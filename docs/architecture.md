...
- Soft deletion runs on that same queue as a lifecycle transaction: it writes the tombstone before moving the body, publishes docs/groups/trash/active as one generation-bound canonical commit, then forces the latest projection before the next queued job. Local autosave is quarantined for the target ids while this transition is in flight.
